...
const AdminAboutPage = () => {
  ...
  const { formData, updateField, saveForm, isSaving, showSuccess } =
  ...
  const removeArtistInfoItem = (id: string) => {
    ...
    updateField(
      "artistInfo",
      artistInfoItems.filter((item) => item.id !== id),
    );
  };
  ...
  // 동적 섹션 관리
  // ──────────────────────────────────────────
  const sections: DynamicSection[] = [...(formData.aboutSections ?? [])].sort(
    (a, b) => a.order - b.order,
  );
      ...
      (formData.aboutSections ?? []).map((s) =>
        s.id === id
          ? {
              ...
              type,
              paragraphs:
                type === "paragraphs" ? (s.paragraphs ?? [""]) : undefined,
              items:
                type === "philosophy-items"
                  ? (s.items ?? [
                      { id: crypto.randomUUID(), quote: "", description: "" },
      ...
      (formData.aboutSections ?? []).map((s) => {
        ...
        return {
          ...
          items: (s.items ?? []).filter(
            (item: PhilosophyItem) => item.id !== itemId,
          ),
        };
      }),
    );
            ...
            <AdminCard key={item.id}>
              ...
              <div className="space-y-3">
                <FormInput
                  ...
                  value={item.key}
                  onChange={(value) =>
                    updateArtistInfoItem(item.id, "key", value)
                  }
                />
                <FormInput
                  label="VALUE"
                  value={item.value}
                  onChange={(value) =>
                    updateArtistInfoItem(item.id, "value", value)
                  }
                />
              </div>
            </AdminCard>
                  ...
                  {(section.paragraphs ?? []).map((para, idx) => (
                    <div key={idx} className="flex items-start gap-2">
                      <FormTextarea
                        ...
                        value={para}
                        onChange={(value) =>
                          updateParagraph(section.id, idx, value)
                        }
                        rows={3}
                      />
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {(section.items ?? []).length > 1 && (
                          <button
                            type="button"
                            onClick={() =>
                              removePhilosophyItem(section.id, item.id)
                            }
