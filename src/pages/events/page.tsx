    ...
    >
      ...
      <div className="space-y-6">
        ...
        {upcomingEvents.length === 0 ? (
          <p className="text-sm text-[var(--color-secondary)] opacity-40">{t('msg_no_items')}</p>
        ) : (
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs">
                      <span className="text-[var(--color-secondary)] opacity-50">{event.venue}</span>
                      {event.location && (
                        <>
                          <span className="text-[var(--color-secondary)] opacity-25">·</span>
                          <span className="text-[var(--color-secondary)] opacity-40">{event.location}</span>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="text-right space-y-1 shrink-0">
                    <p className="text-sm font-mono text-[var(--color-secondary)] opacity-80">{event.date}</p>
                    {event.time && (
                      <p className="text-xs font-mono text-[var(--color-secondary)] opacity-40">{event.time}</p>
                    )}
                  </div>
                </div>
              </div>
